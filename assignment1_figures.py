...
Kør: python assignment1_figures.py
Output: docs/assignment1/
"""
...
SHORT = {
    "ASSAULT": "Assault", "LARCENY THEFT": "Larceny",
    "BURGLARY": "Burglary", "ROBBERY": "Robbery",
    "VANDALISM": "Vandalism", "DRUG OFFENSE": "Drug Off.",
    "MOTOR VEHICLE THEFT": "MV Theft", "WARRANT": "Warrant"
}


# ── Fig 1 ─────────────────────────────────────────────────────────────────────
# No external legend — annotate lines directly at rightmost point
...
for crime in FOCUS:
    ...
    fig1.add_trace(go.Scatter(
        ...
        name=SHORT.get(crime, crime.title()),
        line=dict(color=cmap[crime], width=2), marker=dict(size=5),
...
for crime in FOCUS:
    ...
    fig1.add_annotation(
        ...
        text=f"  {SHORT.get(crime, crime.title())}",
...
fig1.update_layout(**base_layout(
    ...
    xaxis=dict(title="Year", dtick=2, showgrid=True, gridcolor=RULE, range=[2002, 2028]),
    yaxis=dict(title="Number of incidents", showgrid=True, gridcolor=RULE),
    showlegend=False, hovermode="x unified", height=460,
    margin=dict(t=70, b=60, l=70, r=120)
))
save(fig1, "fig1_temporal.html")


# ── Fig 2 ─────────────────────────────────────────────────────────────────────
# English axis labels, colorbar title on top to avoid overlap
...
pivot  = counts.pivot(index="district", columns="category", values="ratio").fillna(0)
...
fig2 = go.Figure(go.Heatmap(
    ...
    colorbar=dict(
        title=dict(text="Ratio", side="top"),
        tickvals=[0, 1, 2, 3],
        ticktext=["0", "1 (avg)", "2", "3"],
        thickness=15
    )
))
fig2.update_layout(**base_layout(
    title=dict(text="Crime Over-representation per Police District", font=dict(size=15)),
    xaxis=dict(title="Crime category", tickangle=-30),
    yaxis=dict(title="Police district"),
    height=520, margin=dict(t=70, b=120, l=130, r=100)
))
...
fig3.update_layout(**base_layout(
    title=dict(text="Time-of-Day Distribution per Crime Category", font=dict(size=15)),
...
# ── Fig 4 ─────────────────────────────────────────────────────────────────────
# No legend — annotate directly on the lines
...
fig4.add_annotation(
    x=ranks[8], y=loc_n[8],
    text="Observed", showarrow=False,
    xanchor="left", yanchor="bottom",
    font=dict(size=11, color=BLUE),
    bgcolor="rgba(250,250,248,0.85)", borderpad=3
)
fig4.add_annotation(
    x=ranks[500], y=fit_y[500],
    text=f"Power-law fit (α = {-coef[0]:.2f})", showarrow=False,
    xanchor="left", yanchor="top",
    font=dict(size=11, color=RED),
    bgcolor="rgba(250,250,248,0.85)", borderpad=3
)
fig4.update_layout(**base_layout(
    title=dict(text="Spatial Power Law: Crime Concentration in SF", font=dict(size=15)),
    xaxis=dict(title="Location rank", type="log", showgrid=True, gridcolor=RULE),
    yaxis=dict(title="Number of incidents", type="log", showgrid=True, gridcolor=RULE),
    height=430
))
...
# ── Fig 5 ─────────────────────────────────────────────────────────────────────
# R² placed at bottom-left using actual data min values
...
for i, c1 in enumerate(crimes4):
    for j, c2 in enumerate(crimes4):
        ...
        fig5.add_trace(go.Scatter(
            ...
            showlegend=False,
            hovertemplate=f"{c2.title()} vs {c1.title()}<br>R²={r2:.2f}<extra></extra>"
        ), row=i + 1, col=j + 1)
